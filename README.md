...
- Хотите адаптивно (решает Susanin: что в VPN, что напрямую) — **не** привязывайте
  устройство/сегмент к VPN-политике в Keenetic; оставьте базовую политику.
- Хотите «всё и всегда через VPN» — можно оставить NDM-политику, но тогда
  `vpn_never` для этого клиента не сработает.
...
- привязка клиента/сегмента к VPN-политике Keenetic («Приоритеты подключений»)
  перекрывает Susanin — для такого клиента `vpn_always`/`vpn_never` не действуют
