...
- `soft` — режим экономии носителя (установщик включает его при установке во
  внутреннюю память NAND): лог-файл не ведётся, состояние не сохраняется,
  бэкапы не создаются. Меньше износа памяти, но список выученных адресов между
  перезагрузками не сохраняется — обучение начинается заново.

Это **выбор режима, а не свойство носителя**: при желании можно поставить
`disk_mode=normal` и на внутренней памяти (будут записи), или `soft` на флешке.
