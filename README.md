...
| `learn_exclude_ports` | порты, которые **не** участвуют в автообучении (скан-шум) | `22,23,53,135,137,138,139,445,554,…` |
...
`learn_exclude_ports` влияет **только на автообучение**: потоки на перечисленные
порты не детектируются и не пополняют `ok`-кэш (так отсекается скан-шум). Само
прохождение трафика не меняется: цепочка `SUSANIN` обрабатывает все порты, а
адреса из `vpn_always.txt` / `vpn_never.txt` и уже выученные адреса
маршрутизируются как обычно, независимо от порта.

### Несколько VPN-подключений (фейловер)

Если указать несколько egress (`egress_interface=nwg0,nwg1`, адреса — по индексу),
