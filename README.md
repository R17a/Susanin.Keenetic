...
Что реально пинится из `vpn_always.txt` / `vpn_never.txt` (где нет A-записей, где
не резолвится, есть ли конфликты между списками) — покажет:

```sh
sh /opt/susanin/tools/check-lists.sh
```
