...
- `normal` — обычный режим (по умолчанию при установке на USB/флешку): ведётся
  лог-файл, состояние сохраняется раз в 5 минут, делаются бэкапы/архивы.
- `soft` — режим экономии носителя (установщик включает его при установке во
  внутреннюю память NAND): лог-файл не ведётся, бэкапы/архивы не создаются, а
  состояние сохраняется редко — раз в `soft_state_interval` (по умолчанию 12 часов).

Это **выбор режима, а не свойство носителя**: `soft` можно включить и на флешке,
`normal` — и на внутренней памяти.
