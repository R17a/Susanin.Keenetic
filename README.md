...
- Хотите адаптивно (решает Susanin: что в VPN, что напрямую) — у клиента/сегмента
  **не должно быть назначено никакой политики** (системная «по умолчанию»).
  Важно: это касается **любой** политики, а не только VPN. Политика вида
  «BASE_ONLY» / «только напрямую» (default через PPPoE) — тоже политика: она
  ставит метку NDM и **тоже перекрывает Susanin**, из-за чего, например, YouTube
  напрямую не открывается, а Susanin не может его завернуть в VPN.
