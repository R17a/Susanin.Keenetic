...
### Режим `disk_mode` (внутренняя память роутера)

Если Entware установлен **во внутреннюю память** (NAND/UBIFS), установщик
выставляет `disk_mode=soft`: лог-файл не ведётся, `susanin.state` не сохраняется,
бэкапы/архивы не создаются — минимум записей на носитель. Цена: кэш обучения не
переживает перезагрузку. Для установки на USB/SD — `disk_mode=normal` (как раньше).
